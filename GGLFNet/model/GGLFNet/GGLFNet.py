...
class Global_Information_Module(nn.Module):
    ...
    def forward(self,x):
        ...
        t1=self.norm1(t1)
        #print(t1.shape)
        x0 = self.conv0T(t1[:,0:65536,:].permute(0,2,1).reshape(B,C,256,256)).contiguous()
        x1 = self.conv1T(t1[:, 65536:81920, :].permute(0,2,1).reshape(B,C, 128, 128)).contiguous()
        x2 = self.conv2T(t1[:, 81920:86016:, :].permute(0,2,1).reshape(B,C, 64, 64)).contiguous()
        x3 = self.conv3T(t1[:, 86016:87040, :].permute(0,2,1).reshape(B,C, 32, 32)).contiguous()
        x4 = self.conv4T(t1[:, 87040:87296, :].permute(0,2,1).reshape(B,C, 16, 16)).contiguous()
...
class GGLFuse(nn.Module):
    def __init__(self, in_channels, out_channels,):
        super(GGLFuse, self).__init__()
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        self.Fuse=GGLFuse(in_channels//3,in_channels//3)
...
class NewNet(nn.Module):
    def __init__(self, n_classes, n_channels=3,dim=64, bilinear=True):
        ...
        self.Global_ifo=Global_Information_Module(dim=dim)
